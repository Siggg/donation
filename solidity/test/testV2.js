...
contract("DonationV2", function(accounts) {
	...
	it("register two account", function() {
		return DonationV2.deployed().then(function(instance){
			...
			return instance.registerBeneficiary(ADDR_BENEF1, {from: ADDR_CERTIFIER});
			...
			return don.registerBeneficiary(ADDR_BENEF2, {from: ADDR_CERTIFIER});
		}).then(function(result){
			//console.log("Got confirmation", result.receipt, "...");
		}).then(function(){
			return	don.getBeneficiaryCount.call({"from": ADDR_DEPLOYER});
	...
	it("unregister an existing account and register it again", function() {
		return DonationV2.deployed().then(function(instance){
			...
			return instance.unregisterBeneficiary(ADDR_BENEF1, { from: ADDR_CERTIFIER});
		}).then(function(result){
			//console.log("Got confirmation", result.receipt, "...");
		}).then(function(){
			return	don.getBeneficiaryCount.call({"from": ADDR_DEPLOYER});
			...
			console.log("Attempt registerBeneficiary", ADDR_BENEF1);
			return don.registerBeneficiary(ADDR_BENEF1, {from: ADDR_CERTIFIER});
		}).then(function(result){
			//console.log("Got confirmation", result.receipt, "...");
		}).then(function(){
			return	don.getBeneficiaryCount.call({"from": ADDR_DEPLOYER});
	...
	it("donate " + amount + " wei", function() {
		return DonationV2.deployed().then(function(instance){
			...
			return don.sendTransaction({value: amount, from: ADDR_DEPLOYER, to: DonationV2.address});
		}).then(function(result){
			//console.log("Got confirmation", result.receipt, "...");
